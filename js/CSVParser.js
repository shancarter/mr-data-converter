...
var CSVParser = {
  ...
  parse: function (input, headersIncluded, delimiterType, downcaseHeaders, upcaseHeaders, decimalSign) {
    ...
    //count the number of semicolon
    RE = new RegExp("[^;]", "gi");
    var numSemicolon = input.replace(RE, "").length;

    var rowDelimiter = "\n";
    //set delimiter
    var columnDelimiter = ",";
    if (numTabs > numCommas && numTabs > numSemicolon) {
      columnDelimiter = "\t"
    }else if (numSemicolon > numCommas && numSemicolon>numTabs) {
      columnDelimiter = ";"
    };
