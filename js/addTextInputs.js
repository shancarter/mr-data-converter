  ...
  $(document).ready(function() {
    ...
    if (boolSelected == false) {
      ...
      for (i = 0; i < targets.length; i++) {
        strInputs += '<tr><td><input style="width:30%" type="text" id="target_' + i.toString() + '" value="' + targets[i].toString().replace(/</g, '&lt;').replace(/>/g, '&gt;') + '" readonly /></td>' 
          + '<td><select style="width:30%" id="mapping_' + i.toString() + '">'
          + headerOptions
          + '</select></td></tr>';
      };
      strInputs += '</table>';
